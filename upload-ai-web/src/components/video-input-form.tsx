...
type Status = 'waiting' | 'converting' | 'uploading' | 'generation' | 'sucess'
...
export function VideoInputFrom(props: VideoUploaded) {
    ...
    return (
        <form onSubmit={handleUploadVideo} className="space-y-6" >
            <label htmlFor="video" className="relative border flex rounded-md aspect-video cursor-pointer border-dashed text-sm flex-col gap-2 items-center justify-center text-muted-foreground hover:bg-primary/5">
                {previewURL ? (
                    <>
                        <video src={previewURL} controls={false} className=" rounded-md pointer-events-none absolute inset-0" />
                    </>
                ) : (
                    <>
            ...
            <div className="space-y-2">
                ...
                <Textarea disabled={status != 'waiting'} ref={promptInputRef} id="transcription_prompt" className="min-h-20 leading-relaxed" placeholder="inclua palavras chaves separadas por virgula" />
            </div>
